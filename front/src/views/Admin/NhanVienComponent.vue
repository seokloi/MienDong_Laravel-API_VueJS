	...
	<div class="callback-form contact-us" v-if="user.id_ChucVu == 1">
		<div class="container">
			<div class="row">
				...
				<div class="alert alert-danger" role="alert" v-if="errors.NgayBatDauHopDong">
					{{ errors.NgayBatDauHopDong[0] }}
				</div>
				<div class="alert alert-danger" role="alert" v-if="errors.id_Quay">
					{{ errors.id_Quay[0] }}
				</div>
				<table class="table table-striped table-bordered table-hover" >
					...
					<tbody>
						<tr class="odd gradeX" align="center">
							...
							<td v-if="!selectedProduct">
								<input v-model="product.email" class="form-control" name="email" />
							</td>
							<td v-else="">
								<input v-model="selectedProduct.email" class="form-control"/>
							</td>
							...
							<td v-if="!selectedProduct">
								<input v-model="product.NgayBatDauHopDong" type="date" class="form-control" name="NgayBatDauHopDong" />
							</td>
							<td v-else="">
								<input type="date" v-model="selectedProduct.NgayBatDauHopDong" class="form-control" disabled=""/>
							</td>

							<td v-if="!selectedProduct">
								<select v-model="product.id_Quay" name="id_Quay" class="form-control">
									<option v-for="quay in listQuays.data" :key="quay.id" :value="quay.id">{{ quay.Ten }}</option>
								</select>
							</td>
							<td v-else="">
								<select v-model="selectedProduct.id_Quay" class="form-control">
									<option v-for="quay in listQuays.data" :key="quay.id" :value="quay.id">{{ quay.Ten }}</option>
								</select>
							</td>
					...
					<transition-group name="slide-fade" tag="tbody">
						<tr v-for="(product, index) in listProducts.data" :key="product.id" class="odd gradeX" align="center">
							...
							<td>{{ product.SDT }}</td>
							<td>{{ product.NgayBatDauHopDong | formatDay }}</td>
							<td>{{ product.Quay }}</td>
							<td>
...
export default {
    name: "Admin-NhanVien",
       ...
       methods: {
           async getListQuays() {
               try {
                   const response = await axios.get(process.env.VUE_APP_API_URL + 'quay')
                   this.listQuays = response.data
               } catch (error) {
           ...
           async getListProducts(page = 1) {
               try {
                   const response = await axios.get(process.env.VUE_APP_API_URL + 'nhanvien', {
                       params: {
                           page: page
                       }
                   })
			...
			async updateProduct() {
                try {
                    await axios.put(process.env.VUE_APP_API_URL + 'nhanvien/' + this.selectedProduct.id, {
                       ...
                       NgayBatDauHopDong: this.selectedProduct.NgayBatDauHopDong,
                       password: this.selectedProduct.password,
                       id_Quay: this.selectedProduct.id_Quay
                    })
                    location.reload()
                } catch (error) {
                    this.error = error.response.data
            ...
            async deleteProduct(product, index) {
                try {
                    await axios.delete(process.env.VUE_APP_API_URL + 'nhanvien/' + product.id)
