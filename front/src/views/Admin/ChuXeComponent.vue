				...
				<table class="table table-striped table-bordered table-hover" >
					...
					<tbody>
						<tr class="odd gradeX" align="center">
							...
							<td v-else="">
								<input type="text" v-model="selectedProduct.DiaChi" class="form-control"/>
							</td>
							...
							<td v-else="">
								<input type="number" v-model="selectedProduct.SDT" class="form-control"/>
							</td>
					...
					<transition-group name="slide-fade" tag="tbody">
						<tr v-for="(product, index) in listProducts.data" :key="product.id" class="odd gradeX" align="center">
							<td>
								<router-link :to = "{ path: '/admin-chuxe_benxe', query: {id_ChuXe: product.id}} ">
									{{ product.Ten }}
								</router-link>
							</td>
							<td>{{ product.email }}</td>
  ...
  name: "Admin-ChuXe",
       ...
       methods: {
           ...
           async getListProducts(page = 1) {
               try {
                   const response = await axios.get(process.env.VUE_APP_API_URL + 'chuxe?page=' + page)
            ...
            async createProduct() {
               try {
                   this.error = null
                   await axios.post(process.env.VUE_APP_API_URL + 'chuxe', {
			...
			async updateProduct() {
                try {
                    await axios.put(process.env.VUE_APP_API_URL + 'chuxe/' + this.selectedProduct.id, {
            ...
            async deleteProduct(product, index) {
                try {
                    await axios.delete(process.env.VUE_APP_API_URL + 'chuxe/' + product.id)
